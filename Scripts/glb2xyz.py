...
def extract_data_from_gltf(file_path):
    ...
    def get_buffer_data(buffer, buffer_view, accessor, dtype, num_components):
        buffer_bytes = buffer[buffer_view.byteOffset: buffer_view.byteOffset + buffer_view.byteLength]
        data = np.frombuffer(
            buffer_bytes,
            dtype=dtype,
            count=accessor.count * num_components,  # 3 components per vertex (e.g., x, y, z)
    ...
    positions = get_buffer_data(buffer, positions_buffer_view, positions_accessor, np.float32, 3)
    colors = get_buffer_data(buffer, colors_buffer_view, colors_accessor, np.uint8, 4)  # Assuming VEC4 for colors
